...
def extract_json_array(text: str) -> Optional[list]:
    """Extracts a JSON array [] from text. Used for flowchart label parsing."""
    try:
        text = re.sub(r"```json|```", "", text).strip()
        start = text.find("[")
        end = text.rfind("]") + 1
...
def generate_flowchart(plan: dict) -> str:
    """
    Step 1: Ask LLM for short node labels only (no Mermaid syntax).
    Step 2: Inject labels into a fixed Mermaid template deterministically.
    Returns a valid Mermaid flowchart string.
    """
    steps = plan.get("plan", [])

    # ── Step 1: LLM generates short labels only ──────────────────
    prompt = f"""You are given a list of DIY project steps.
Summarize each step as a SHORT label of maximum 5 words.
Return ONLY a JSON array of strings, one per step.
Do not include step numbers. Do not include Mermaid syntax.

STEPS:
{json.dumps(steps, indent=2)}

Example output format:
["Install Alfred app", "Create account", "Grant permissions", "Enable motion detection", "Test system"]
...
Return ONLY the JSON array. Nothing else."""
    ...
    try:
        ...
        # ── Use extract_json_array, not extract_json ──
        labels = extract_json_array(text)

        if not isinstance(labels, list):
            raise ValueError("LLM did not return a list")

        # Sanitize: truncate, strip special chars that break Mermaid
        labels = [
            re.sub(r'["\[\]{}|]', '', str(label).strip())[:40]
            for label in labels
        ]

    except Exception as e:
        # Fallback: use truncated plan steps directly
        print(f"   ⚠️  Flowchart label generation failed: {e}. Using fallback labels.")
        labels = [s[:40] for s in steps]

    # ── Step 2: Deterministic Mermaid assembly ────────────────────
    return _build_mermaid(labels)
...
- STEP LIMIT: Generate a MAXIMUM of 6 steps for standard projects, 8 steps for complex projects.
  Merge related actions into one step where possible. Fewer steps is always better.
  Difficulty level is: {project.get('difficulty', 'Standard')}
...
7. STEP COUNT: Does the plan exceed 8 steps? If so, mark invalid and suggest which
   steps to merge. For non-Hard difficulty, flag if it exceeds 6 steps.
...
def run_plan_loop(project: dict, device: dict) -> dict:
    ...
    final_plan["mermaid_chart"] = generate_flowchart(final_plan)

    # ── Fetch project-level YouTube video ──
    final_plan["video_url"] = fetch_project_video(
        title=project.get("title", ""),
        goal=final_plan.get("goal", ""),
        device_name=device["device_name"],
        sensor_used=final_plan.get("sensor_used", []),
    )
...
def fetch_project_video(title: str, goal: str, device_name: str, sensor_used: list) -> Optional[str]:
    """
    Uses Tavily to find the most relevant YouTube tutorial.
    Returns the first valid YouTube URL from results, or None.
    """
    from app.tools.web_search import web_search

    BRAND_WORDS = {"apple", "samsung", "google", "pixel", "iphone", "android", "huawei", "oneplus"}
    core_title = " ".join(w for w in title.split() if w.lower() not in BRAND_WORDS)

    query = f"{core_title} DIY tutorial site:youtube.com"
    print(f"\n🎬 Tavily YouTube search: '{query}'")
    ...
    if not results:
        print("   ℹ️  No results from Tavily")
        return None

    # Extract all YouTube URLs from Tavily response
    ...
    for url in youtube_urls:
        clean = url.split("&")[0]  # strip tracking params like &t=30s
        if clean not in seen:
            seen.add(clean)
            unique_urls.append(clean)

    if not unique_urls:
        print("   ℹ️  No YouTube URLs found in Tavily results")
        return None

    # Return first result — Tavily already ranks by relevance
    best = unique_urls[0]
...
def _attach_video(result: dict, step: str, device_name: str, step_number: int) -> dict:
    """Placeholder — per-step video recommendation not yet implemented."""
